var email = document.querySelector('#email'),
    ...
    peci = document.querySelector('.peci');
...
function getCoord(e) {
    ...
    var eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, caretPos, emailCoords.y + 25);
    var eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, caretPos, emailCoords.y + 25);
    var noseAngle = getAngle(noseCoords.x, noseCoords.y, caretPos, emailCoords.y + 25);
    var mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, caretPos, emailCoords.y + 25);
    ...
    // Kacamata ikut bergerak bersama wajah
    if (glasses) {
        TweenMax.to(glasses, 1, { x: -faceX, y: -faceY, skewX: -faceSkew, transformOrigin: "center center", ease: Expo.easeOut });
    }

    // Peci ikut bergerak bersama kepala
    if (peci) {
        TweenMax.to(peci, 1, { x: hairX, scaleY: hairS, transformOrigin: "center bottom", ease: Expo.easeOut });
    }
...
function resetFace() {
    ...
    // Reset kacamata
    if (glasses) {
        TweenMax.to(glasses, 1, { x: 0, y: 0, skewX: 0, ease: Expo.easeOut });
    }

    // Reset peci
    if (peci) {
        TweenMax.to(peci, 1, { x: 0, y: 0, scaleY: 1, ease: Expo.easeOut });
    }
}
